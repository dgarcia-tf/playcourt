...
function createDashboardMatchListItem(
  match,
  { includeScope = false, displayStatus = false, showReservationAction = false } = {}
) {
  ...
  if (displayStatus) {
    ...
    const statusLabel = STATUS_LABELS[statusKey] || STATUS_LABELS[match.status] || match.status;
    if (statusLabel) {
      const statusTag = document.createElement('span');
      statusTag.className = `tag status-${statusKey || match.status}`;
  ...
  } else if (displayStatus && match.proposal?.proposedFor) {
    const proposer = match.proposal?.requestedBy?.fullName || 'Un jugador';
    const proposalLabel = formatDate(match.proposal.proposedFor);
    meta.appendChild(document.createElement('span')).textContent = proposalLabel
      ? `${proposer} propuso ${proposalLabel}`
      : `${proposer} propuso un horario.`;
    if (match.court) {
  ...
  if (showReservationAction && !match.scheduledAt && matchId) {
  ...
  const fragment = document.createDocumentFragment();
    const item = createDashboardMatchListItem(match, { includeScope });
    if (item) {
...
function renderDashboardMyMatches(matches = []) {
  ...
  sortedMatches.forEach((match) => {
    const item = createDashboardMatchListItem(match, {
      includeScope: true,
      displayStatus: true,
