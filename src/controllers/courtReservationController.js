...
async function getAvailability(req, res) {
  ...
  const normalizedReservationType = (() => {
    if (!requestedReservationType) {
      return RESERVATION_TYPES.MANUAL;
    }

    if (reservationTypeCandidates.includes(requestedReservationType)) {
      return requestedReservationType;
    }

    if (requestedReservationType.toLowerCase() === 'match') {
      return RESERVATION_TYPES.MATCH;
    }
